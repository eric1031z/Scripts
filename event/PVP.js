...
function setup(mapid) { //mapid = ((level, 0-3) (type, 0-3) (mapid)) in one string
    ...
    eim.setProperty("lvl", mapid.split(" ")[0]); //新手,超新..
...
function playerEntry(eim, player) {
	...
	var to = eim.getPlayers().get(0).getName();
	...
	eim.broadcastPlayerMsg(-7, player.getName() + " 玩家已加入戰鬥！");
	
    var i = getMaxPlayerCount(type);
    broadcastType(eim, player);
    if (eim.getProperty("started").equals("0")) {
        eim.broadcastPacket(CWvsContext.getMidMsg("目前共 " + eim.getPlayerCount() + "/" + i + " 位玩家！", true, 1));
        eim.broadcastPacket(CWvsContext.getMidMsg("目前正在招募玩家中。", true, 0));
...
function doItemDrop(eim) {
    ...
    for (var i = 0; i < num; i++) {
        ...
        if (r != 2900049 && r != 2900043 && r != 2900042 && r != 2900039 && r != 2900038 && r != 2900037 && r != 2900029 && r != 2900016 && r != 2900015 && r!= 2900006 && r!=290007) {
            map.spawnAutoDrop(r, new java.awt.Point(d.get(Math.floor(Math.random() * d.size())).getPoint1()));
        }
    }
    eim.schedule("doItemDrop", 5000);
}
...
function playerDead(eim, player) {
    ...
    player.getClient().getSession().write(CField.getPVPKilled(player.getName() + " 已被擊敗！"));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
        eim.setProperty("red", "99999");
        eim.broadcastPlayerMsg(-7, "冰騎士已被擊敗！");
        end(eim);
        return;
    }
    var type = parseInt(eim.getProperty("type"));
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        eim.broadcastPacket(CField.getPVPScoreboard(c, ty)); //就是你這條狗
        if (ty != 0 && ty != 2) {
		...
		eim.schedule("updateScoreboard", 1000);
    } else {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players.get(i).getClient().getSession().write(CField.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1)); //ok
        }
    }
}
...
function playerRevive(eim, player) {
	...
	eim.broadcastPacket(CField.enablePVP(true)); //ok
    return true;
}

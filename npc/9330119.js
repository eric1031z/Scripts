...
function action(mode, type, selection) {
    ...
    if (status == -1) {
		...
		var msg = "";
		msg += icon1 + " #b當前普轉數量 : #k#r" + a + "#k\r\n" + icon1 + " #b本轉蛋機須#k #r70等#k #b才可使用\r\n"; 
        if (cm.haveItem(ticket) && cm.getPlayer().getLevel() >= 70) {
			cm.sendSimple(icon5 + " #d#e【普通轉蛋機】#k\r\n" + msg + "#L0#" + icon3 + " 我要轉蛋\r\n#L1#" + icon3 + " 查看轉蛋機內容物\r\n"  + (cm.getPlayer().isGM() ? ("\r\n#L2#" + icon3 + " 更新轉蛋機" ) : ""));
        } else {
	  ...
	  if(se == 0){
        ...
        //cm.dispose();
		  ...
		  var x = new Array(1112765,1112769,1112773,1112777,1132194,1132198,1132202,1132206);
		  for(var i = 0 ; i < x.length ; i++){
			  if(MapleItemInformationProvider.getInstance().itemExists(x[i]) ){
				  if(!MapleItemInformationProvider.getInstance().isCash(x[i]) /*&& MapleItemInformationProvider.getInstance().getReqLevel(i) < 150 && MapleItemInformationProvider.getInstance().getEquipById(i).getWatk() > 0 && MapleItemInformationProvider.getInstance().getEquipById(i).getWatk() <= 10*/){
					   cm.gachaponadditem(x[i],8000,1,1);
					   a++;
				  }   

...
function start() {
	 if (cm.getPlayer().getOneTimeLog(todayp + "女皇") >= 5 && cm.getPlayer().getMapId() != 271040100){
    ...
    if (cm.getPlayer().getLevel() < 170) {
        cm.sendOk("未滿170級無法挑戰。");
        cm.dispose();
        return;
    }
    if (cm.getPlayer().getClient().getChannel() > 30) {
        cm.sendOk("請至#b第1-10頻道#k挑戰西格諾斯。");
        cm.dispose();
        return;
    }
            ...
            if (mode == 1) {
                if (selection == 0) {
                    ...
                    if (cm.getSquad("Cygnus") != null) {
						...
						var x = cm.getSquad("Cygnus").getMembers().iterator();
			            while(x.hasNext()){
				            var p = x.next();
				            var chr = MapleCharacter.getCharacterByName(p);
				            chr.setOneTimeLog(todayp + "女皇");
			            }				

...
function start() {
        if (cm.getPlayer().getOneTimeLog(todayp + "皮卡") >= 10){
			cm.sendOk("一天只能打10次喔! 12點重製!");
			cm.dispose();
			return;
		}
		...
		if (cm.getPlayer().getClient().getChannel() > 10/*cm.getPlayer().getClient().getChannel() != 5*/) {
			cm.sendOk("請至#第1-10頻道#k挑戰皮卡啾。");
			cm.dispose();
			return;
		}
...
function action(mode, type, selection) {
	...
	case 10:
		...
		if (selection == 0) {
			...
			var x = cm.getSquad("PinkBean").getMembers().iterator();
			while(x.hasNext()){
				var p = x.next();
				var chr = MapleCharacter.getCharacterByName(p);
				chr.setOneTimeLog(todayp + "皮卡");
			}				

...
function start() {
	...
	if (cm.getPlayer().getOneTimeLog(todayp + "凡雷") >= 5){
			cm.sendOk("一天打5次喔! 12點重製!!!");
			cm.dispose();
			return;
	}
		if (cm.getPlayer().getLevel() < 120) {
			cm.sendOk("未滿120級無法挑戰凡雷恩。");
			cm.dispose();
			return;
		}
		if (cm.getPlayer().getClient().getChannel() > 10/*cm.getPlayer().getClient().getChannel() != 7 && cm.getPlayer().getClient().getChannel() != 8 && cm.getPlayer().getClient().getChannel() != 9*/) {
			cm.sendOk("請至#b第1頻道#k挑戰凡雷恩。");
			cm.dispose();
			return;
		}
...
function action(mode, type, selection) {
	...
	case 10:
		...
		if (selection == 0) {
			...
			var x = cm.getSquad("VonLeon").getMembers().iterator();
			while(x.hasNext()){
				var p = x.next();
				var chr = MapleCharacter.getCharacterByName(p);
				chr.setOneTimeLog(todayp + "凡雷");
			}			

...
function start() {
	var yy = cm.getPlayer().isGM() ? 30 : 30;
	    if (cm.getPlayer().getClient().getChannelServer().getMapFactory().getMap(240060200).getCharactersSize() > 0){
			cm.sendOk("裡面有人");
			cm.dispose();
		}
	    if (cm.getPlayer().getOneTimeLog(todayp + "龍王") >= yy){
			cm.sendOk("一天打30次喔! 12點重製!!!");
			cm.dispose();
			return;
		}
		...
		if (cm.getPlayer().getClient().getChannel() > 20/*cm.getPlayer().getClient().getChannel() != 4 && cm.getPlayer().getClient().getChannel() != 5*/) {
			cm.sendOk("請至#b第1-10頻道#k挑戰。");
			cm.dispose();
			return;
		}
...
function action(mode, type, selection) {
	...
	case 10:
		...
		if (selection == 0) {
			...
			var x = cm.getSquad("Horntail").getMembers().iterator();
			while(x.hasNext()){
				var p = x.next();
				var chr = MapleCharacter.getCharacterByName(p);
				chr.setOneTimeLog(todayp + "龍王");
			}				

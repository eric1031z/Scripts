...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		for(var a = 0 ; a < pack.length ; a++){
			msg += "#L" + a + "#" + icon3 + "#b論壇獎勵#k #d 領取獎勵#k\r\n";
		}
		if(cm.getPlayer().getPrizeLog("論" + todayp) > 0){
		    cm.sendSimple(cm.getPlayer().getPrizeLog("論壇" + todayp) > 0 ? "#e您獲得過此獎勵" : msg);
		}else{
			cm.sendOk("您並未在論壇推廣名單中");
			cm.dispose();
		}
	}else if(status == 1){
		this. sel = selection;
		...
		if(checkitems > 1){
			...
			cm.sendOk("感謝您的推廣!");
			cm.getPlayer().setPrizeLog("論壇" + todayp);
			cm.dispose();
		}
	}
}

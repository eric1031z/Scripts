...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		for(var a = 0 ; a < pack.length ; a++){
			msg += "#L" + a + "#" + icon3 + "#b商店補償1#k #d 領取補償#k\r\n";
		}
		if(cm.getPlayer().getPrizeLog("商1") > 0){
		    cm.sendSimple(cm.getPlayer().getPrizeLog("商1補償") > 0 ? "#e您獲得過此補償" : msg);
		}else{
			cm.sendOk("您並未在商店補償1名單中");
			cm.dispose();
		}
	}else if(status == 1){
		this. sel = selection;

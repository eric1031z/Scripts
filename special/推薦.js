...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if(cm.getPlayer().getPrizeLog("推" + todayp) > 0){
		    cm.sendSimple(cm.getPlayer().getPrizeLog("推薦" + todayp) > 0 ? "#e您獲得過此獎勵" : msg);
		}else{
			cm.sendOk("您並未在推薦名單中");
			cm.dispose();
		}
		...
		if(checkitems > 1){
			...
			cm.sendOk("感謝您的推廣!");
			cm.getPlayer().setPrizeLog("推薦" + todayp);
			cm.dispose();
		}
	}
}

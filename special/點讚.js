...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		for(var a = 0 ; a < pack.length ; a++){
			msg += "#L" + a + "#" + icon3 + "#b讚數獎勵#k #d 領取獎勵#k\r\n";
		}
		if(cm.getPlayer().getPrizeLog("讚") > 0){
		    cm.sendSimple(cm.getPlayer().getPrizeLog("讚數獎勵") > 0 ? "#e您獲得過此獎勵" : msg);
		}else{
			cm.sendOk("您並未在讚數獎勵名單中");
			cm.dispose();
		}
	}else if(status == 1){
		this. sel = selection;

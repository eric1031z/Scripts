...
function action(mode, type, selection) {
	...
	if (status == 0){
		...
		if(s == 0){
			...
			msg += icon3 + " #d潛能可以應用於#r已擁有三排潛能#k#d上,增加為#k#r第四排#k\r\n";
		    ...
		    for(var i = 0 ; i < 96 ; i++){
			    if(cm.getInventory(1).getItem(i) != null && !MapleItemInformationProvider.getInstance().isCash(cm.getInventory(1).getItem(i).getItemId())){
				    if(cm.getInventory(1).getItem(i).getPotential4() > 0){
				         msg += "#L" + i + "#" + icon3 + " #d#t" + cm.getInventory(1).getItem(i).getItemId() + "##k\r\n";
				    }
			    }
		    }
		    if(msg == ""){
			    cm.sendOk("#d您並沒有已裝備的應用四潛裝備喔!#k");
			    status = -1;
		    }else{
		...
		if(s != 4){
			...
			var tocheck = MapleItemInformationProvider.getInstance().getPotentialInfo(cm.getInventory(1).getItem(op).getPotential4()).get(1);
		}
		...
		if(s == 1){
		    ...
		    msg += icon3 + " #d應用效果 :#k #r" +  GameConstants.resolvePotentialID2(MapleItemInformationProvider.getInstance().getReqLevel(cm.getInventory(1).getItem(op).getItemId())/10,cm.getInventory(1).getItem(op).getPotential4()) + "#k\r\n";
		...
		for(var i = 0 ; i < 96 ; i++){
			if(cm.getInventory(1).getItem(i) != null && !MapleItemInformationProvider.getInstance().isCash(cm.getInventory(1).getItem(i).getItemId())){
				if(cm.getInventory(1).getItem(i).getPotential3() > 0 && cm.getInventory(1).getItem(i).getPotential4() == 0){
				    msg += "#L" + i + "#" + icon3 + " #d#t" + cm.getInventory(1).getItem(i).getItemId() + "##k\r\n";
				}
			}
		}
		if(msg == ""){
			cm.sendOk("#d您並沒有符合可應用條件的裝備喔!#k\r\n#r(需已有三牌潛能且無四排應用潛能的非現金裝備)#k");
			status = -1;
		}else{
		    cm.sendSimple(icon5 + " #d請選擇應用的裝備#k\r\n" + msg);	
		}
		}else if(s == 4){
			cm.setPCount(cm.getPlayer().getId(),cm.getInventory(1).getItem(op).getPotential4(),1,grade);
			cm.getInventory(1).getItem(op).setPotential4(0);
		...
		cm.getInventory(1).getItem(k).setPotential4(op);

...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		cm.sendGetNumber("您想要設定編號#r" + op + "#k 效果: #b" + GameConstants.resolvePotentialID2(20,op) +"#k的出現機率為幾%(1%填100)?",0,0,10000);		
	}else if(status == 3){
		this. os = selection;
		cm.setPotentialChance(os,op);
		cm.sendOk("已為您調整編號#r" + op + "#k 效果 : #b" + GameConstants.resolvePotentialID2(20,op) +"#k的出現機率至 -- #r" + (os/100) + "%#k");
		cm.dispose();
	}

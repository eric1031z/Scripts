...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		for(var a = 0 ; a < pack.length ; a++){
			msg += "#L" + a + "#"+icon3+"#b《 #k#r#e" + todayp + " 任務#n#k#b 》#k #d 查看任務通知#k\r\n";
		}
		cm.sendSimple(cm.getPlayer().getPrizeLog(todayp + "任務") > 0 ? "您已完成#r" + todayp + " #k任務" : msg);
	}else if(status == 1){
		this. sel = selection;
		if(sel < 0){
			cm.dispose();
		...
		if(checkitems > 1){
			...
			cm.sendOk("謝謝您的兌換!");
			cm.getPlayer().setPrizeLog(todayp + "任務");
			cm.dispose();
		}
	}
}
